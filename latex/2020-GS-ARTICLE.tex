...
Attraverso una buona tecnica di ripresa stereofonica è possibile costruire
un’informazione sonora multicanale che rappresenti in modo convincente e si
avvicini alla percezione del segnale sonoro reale ripreso. In musica, più che
nella ripresa paesagistica, lo scopo è trasmettere nel miglior modo possibile il
contenuto culturale ed emozionale dell’evento musicale conservandone le modalità
di propagazione nello spazio.

La letteratura specifica descrive le tecniche di ripresa stereofonica
esclusivamente mediante applicazioni microfoniche pratiche derivandole da
logiche industriali discografiche. Questo tipo di narrazione più che analizzare
i principi di funzionamento per ampliare la comprensione del modello prima
dell'implementazione limita il ragionamento a fatti pratici rapportando
semplicisticamente ripresa a diffusione, in un sistema apparentemente di
riferimento.

\begin{quote}
Modern stereophonic recording, or stereo as we normally call it, makes use of
many diverse microphone arrays and techniques. At the basis of them all are a
set of fundamental two- or three-microphone arrays for picking up a stereo sound
stage for reproduction over a pair of loud- speakers. In stereo reproduction the
listener is able to perceive images on the stereo sound stage which may span the
entire angular width of the loudspeaker array. \cite{je:05}%The sound sources that are
%perceived between the loudspeakers are known as “phantom images,” because they
%appear at positions where there are no physical, or real sources of sound.
\end{quote}

Sistemi di due o tre microfoni riprodotti su coppie di diffusori sono \emph{una}
possibile implementazione stereofonica, \emph{non l'unica}, non necessariamente
a base due, come vedremo in seguito.

Inoltre non esiste in letteratura \cite{je:05} \cite{gb:09} \cite{bb:98}
\cite{fr:21} una trattazione matematica delle caratteristiche di ripresa né una valutazione analitica delle
...
si può vedere nella fig. \ref{blumlein} , mentre il microfono corrispondente al canale
...
La problematica rilevata da Gerzon, è nell'ascolto su un sistema quadrifonico, che prevede
i due canali LR e la somma sul fronte e la sottrazione nella parte posteriore. 
Essendo la ripresa effettuata con una coppia Blumlein debole nella cattura di ciò che 
avviene posteriormente ai microfoni,  questo sistema si rivela sconveniente nell'emissione dei segnali acustici dall'altoparlante
posteriore, tanto da risultare più efficace il tenerlo spento.
La soluzione proposta da Gerzon quindi, consiste nel disporre quattro altoparlanti: due frontali e due posteriori posti a coppie
una a sinistra ed una a destra che emettono segnali acustici secondo la seguente distribuzione:
A = 0.924 L – 0.383R
B = 0.924 L + 0.383 R
C = 0.383 L + 0.924 R
D = -0.383 L + 0.924 R
